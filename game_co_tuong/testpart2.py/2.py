...
class CoTuongGUI:
    def __init__(self, root):
        ...
        # ĐÃ SỬA: Thay đổi đúng tên file ảnh gốc của bồ để tránh lỗi không tìm thấy file
        self.bg_image = None
        if HAS_PIL:
            try:
    ...
    # ================= KHU VỰC QUẢN LÝ GIAO DIỆN MENU =================
    ...
    def man_hinh_bat_dau(self):
        ...
        # Nếu có ảnh nền, chèn ảnh phủ kín khung hình và đặt nút lên trên
        if self.bg_image:
            bg_label = tk.Label(self.khung_giao_dien, image=self.bg_image)
            bg_label.place(x=0, y=0, relwidth=1, relheight=1)
            
            btn_bat_dau = tk.Button(self.khung_giao_dien, text="Bắt Đầu Chơi", font=("Arial", 14, "bold"), bg="#b32d2e", fg="white",
                                    activebackground="#c0392b", activeforeground="white", bd=0, width=20, height=2, cursor="hand2",
                                    command=self.man_hinh_chon_che_do)
            btn_bat_dau.place(relx=0.5, rely=0.75, anchor=tk.CENTER)
        else:
            # Phương án dự phòng nếu thiếu file ảnh hoặc chưa cài Pillow
            btn_bat_dau = tk.Button(self.khung_giao_dien, text="Bắt Đầu Chơi", font=("Arial", 14, "bold"), bg="#b32d2e", fg="white",
                                    activebackground="#c0392b", activeforeground="white", bd=0, width=20, height=2, cursor="hand2",
                                    command=self.man_hinh_chon_che_do)
            btn_bat_dau.pack(pady=(350, 0))

    def man_hinh_chon_che_do(self):
        self.xoa_khung_giao_dien()
        
        tk.Label(self.khung_giao_dien, text="Chọn Chế Độ Chơi", font=("Arial", 20, "bold"), fg="#2c3e50", bg="#fdfaf6").pack(pady=(120, 40))
        
        btn_1vs1 = tk.Button(self.khung_giao_dien, text="⚔️   Chế Độ 1 VS 1   ⚔️\n(Chơi đối kháng offline)", font=("Arial", 14, "bold"),
                             bg="white", fg="#b32d2e", activebackground="#f1f2f6", activeforeground="#b32d2e",
                             bd=2, relief=tk.SOLID, width=28, height=4, cursor="hand2", highlightthickness=0,
    ...
    def man_hinh_nhap_ten(self):
        ...
        tk.Label(self.khung_giao_dien, text="Thông Tin Người Chơi", font=("Arial", 20, "bold"), fg="#2c3e50", bg="#fdfaf6").pack(pady=(80, 30))
        
        frame_p1 = tk.Frame(self.khung_giao_dien, bg="#fdfaf6")
        frame_p1.pack(pady=12)
        ...
        ent_p1 = tk.Entry(frame_p1, textvariable=self.var_ten1, font=("Arial", 12), width=30, bg="#f1f2f6", bd=1, relief=tk.SOLID)
        ent_p1.pack(ipady=8)
        
        frame_p2 = tk.Frame(self.khung_giao_dien, bg="#fdfaf6")
        frame_p2.pack(pady=12)
        ...
        ent_p2 = tk.Entry(frame_p2, textvariable=self.var_ten2, font=("Arial", 12), width=30, bg="#f1f2f6", bd=1, relief=tk.SOLID)
        ent_p2.pack(ipady=8)
        
        self.btn_choi = tk.Button(self.khung_giao_dien, text="Bắt Đầu Trận Đấu", font=("Arial", 13, "bold"),
                                  bg="#dcdde1", fg="#7f8c8d", state=tk.DISABLED, bd=0, width=22, height=2, cursor="arrow")
        self.btn_choi.pack(pady=40)
    ...
    def kiem_tra_form_nhap(self, *args):
        ...
        if ten1 != "" and ten2 != "":
            self.btn_choi.config(state=tk.NORMAL, bg="#b32d2e", fg="white", cursor="hand2", command=self.vao_tran_dau)
        else:
            self.btn_choi.config(state=tk.DISABLED, bg="#dcdde1", fg="#7f8c8d", cursor="arrow")
    ...
    def hien_thi_bang_ket_qua(self, ten_thang, ten_thua, phe_thang, phe_thua, tieu_de, thong_diep):
        ...
        # KHUNG NGƯỜI THUA (Hoặc bên trái khi Hòa)
        ...
        # KHUNG NGƯỜI THẮNG (Hoặc bên phải khi Hòa)
        ...
        btn_tai_dau = tk.Button(popup, text="↺ Tái Đấu (Ván Mới)", font=("Arial", 12, "bold"), bg="#c0392b", fg="white", 
                                activebackground="#e74c3c", activeforeground="white", bd=0, padx=20, pady=10, cursor="hand2", 
                                command=lambda: [self.reset_game(), popup.destroy()])
