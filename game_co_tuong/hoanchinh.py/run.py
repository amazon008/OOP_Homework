...
class NutBoGoc(tk.Canvas):
    ...
    def ve_nut(self):
        ...
        self.text_id = self.create_text(self.width/2, self.height/2, text=self.text, font=self.font, fill=text_color, justify="center")
    ...
    def _on_enter(self, event):
        if self.state == tk.NORMAL:
            ...
            # Đổi màu đậm hơn một xíu khi di chuột vào
            self.itemconfig(self.rect_id, fill="#19b849" if self.mau_nen == "#1cd153" else "#c0392b")
...
class CoTuongGUI:
    ...
    def ve_hinh_bo_goc(self, canvas, x1, y1, x2, y2, radius=25, **kwargs):
        """Hàm thuật toán vẽ hình chữ nhật bo góc trên Canvas"""
        points = [
    ...
    def man_hinh_bat_dau(self):
        ...
        btn_bat_dau = NutBoGoc(self.canvas_bg, "Bắt Đầu Chơi", self.man_hinh_chon_che_do, 
                                mau_nen="#e74c3c", mau_chu="white", mau_bg_cha="#fdfaf6" if not self.bg_image else None, 
                                width=260, height=56, radius=28, font=("Arial", 16, "bold"))
        # Nếu có hình nền, ta đặt nút chìm vào nền (do Tkinter ko hỗ trợ trong suốt hoàn toàn, ta lách luật xíu)
        if self.bg_image:
             self._ve_nut_truc_tiep(self.canvas_bg, 300, 585, 260, 56, 28, "Bắt Đầu Chơi", "#e74c3c", self.man_hinh_chon_che_do)
        else:
             btn_bat_dau.place(relx=0.5, rely=0.75, anchor=tk.CENTER)

    def _ve_nut_truc_tiep(self, canvas, cx, cy, w, h, r, text, color, cmd):
        """Vẽ nút nổi trực tiếp lên nền ảnh để không bị dính nền trắng vuông"""
        x1, y1 = cx - w/2, cy - h/2
        x2, y2 = cx + w/2, cy + h/2
        self.ve_hinh_bo_goc(canvas, x1+3, y1+3, x2+3, y2+3, radius=r, fill="#000000", stipple="gray50")
        btn = self.ve_hinh_bo_goc(canvas, x1, y1, x2, y2, radius=r, fill=color)
        txt = canvas.create_text(cx, cy, text=text, font=("Arial", 16, "bold"), fill="white")
        
        is_pressed = [False]
        def press(e):
            is_pressed[0]=True
            canvas.move(btn, 0, 2); canvas.move(txt, 0, 2)
        def release(e):
            if is_pressed[0]:
                is_pressed[0]=False
                canvas.move(btn, 0, -2); canvas.move(txt, 0, -2)
    ...
    def man_hinh_chon_che_do(self):
        ...
        # VẼ KHUNG CHỨA BO GÓC (Đập bỏ tk.Frame vuông vức cũ)
        fw, fh = 400, 260
        cx, cy = 300, 390
        x1, y1 = cx - fw/2, cy - fh/2
        x2, y2 = cx + fw/2, cy + fh/2
        
        # Đổ bóng nhẹ cho khung
        self.ve_hinh_bo_goc(self.canvas_bg, x1+4, y1+4, x2+4, y2+4, radius=20, fill="#000000", stipple="gray25")
        # Khung nền trắng bo tròn góc siêu mượt
        self.ve_hinh_bo_goc(self.canvas_bg, x1, y1, x2, y2, radius=20, fill="#fdfaf6", outline="#e2e8f0", width=1)
        
        tk.Label(self.canvas_bg, text="Chọn Chế Độ Chơi", font=("Arial", 22, "bold"), fg="#2c3e50", bg="#fdfaf6").place(relx=0.5, rely=0.4, anchor=tk.CENTER)
        
        btn_1vs1 = NutBoGoc(self.canvas_bg, "⚔️   Chế Độ 1 VS 1   ⚔️\n(Chơi đối kháng offline)", self.man_hinh_nhap_ten, 
                             mau_nen="#1cd153", mau_chu="white", mau_bg_cha="#fdfaf6", 
                             width=280, height=64, radius=20, font=("Arial", 12, "bold"))
        btn_1vs1.place(relx=0.5, rely=0.55, anchor=tk.CENTER)
    ...
    def man_hinh_nhap_ten(self):
        ...
        tk.Label(self.canvas_bg, text="Thông Tin Người Chơi", font=("Arial", 22, "bold"), fg="#2c3e50", bg="#fdfaf6").place(relx=0.5, rely=0.3, anchor=tk.CENTER)
        
        # Người chơi 1
        tk.Label(self.canvas_bg, text="Tên Người Chơi 1 (Quân Đỏ):", font=("Arial", 11, "bold"), fg="#e74c3c", bg="#fdfaf6").place(relx=0.5, rely=0.38, anchor=tk.CENTER)
        self.var_ten1 = tk.StringVar()
        self.var_ten1.trace_add("write", self.kiem_tra_form_nhap) 
        
        # ĐÃ ĐỔI MÀU CHỮ: fg="#e74c3c" (Màu đỏ tươi)
        self.ent_p1 = tk.Entry(self.canvas_bg, textvariable=self.var_ten1, font=("Arial", 13, "bold"), justify="center", 
                               bg="#F5F5F5", fg="#e74c3c", relief="flat", highlightthickness=1,
                               highlightbackground="#CCCCCC", highlightcolor="#e74c3c")
        self.ent_p1.place(relx=0.5, rely=0.43, anchor=tk.CENTER, width=300, height=38)
        
        # Người chơi 2
        tk.Label(self.canvas_bg, text="Tên Người Chơi 2 (Quân Đen):", font=("Arial", 11, "bold"), fg="#2c3e50", bg="#fdfaf6").place(relx=0.5, rely=0.51, anchor=tk.CENTER)
        self.var_ten2 = tk.StringVar()
        self.var_ten2.trace_add("write", self.kiem_tra_form_nhap) 
        
        # ĐÃ ĐỔI MÀU CHỮ: fg="#1cd153" (Màu xanh lá giống yêu cầu của bro)
        self.ent_p2 = tk.Entry(self.canvas_bg, textvariable=self.var_ten2, font=("Arial", 13, "bold"), justify="center", 
                               bg="#F5F5F5", fg="#1cd153", relief="flat", highlightthickness=1,
                               highlightbackground="#CCCCCC", highlightcolor="#1cd153")
        self.ent_p2.place(relx=0.5, rely=0.56, anchor=tk.CENTER, width=300, height=38)
        
        self.btn_choi = NutBoGoc(self.canvas_bg, "Bắt Đầu Trận Đấu", self.vao_tran_dau, 
                                 mau_nen="#1cd153", mau_chu="white", mau_bg_cha="#fdfaf6", 
                                 width=260, height=52, radius=26, state=tk.DISABLED)
        self.btn_choi.place(relx=0.5, rely=0.68, anchor=tk.CENTER)
